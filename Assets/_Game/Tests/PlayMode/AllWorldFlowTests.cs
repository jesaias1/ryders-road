...
{
    public sealed class AllWorldFlowTests
    {
        ...
        [UnityTest] public IEnumerator AuthoredSequencesCarrySharedMovement()
        {
            var allFailures=new List<string>();
            foreach(var world in new[]{
                ("module.001.first-steps","m01.",new[]{("arrival","start","restore.approach"),("terraces","restore.approach","patch.base")}),
                ("module.002.moving-parts","m02.",new[]{("ridge","restore.platform","rejoin"),("summit","lift.exit","patch.base")}),
                ("module.003.flow-error","m03.",new[]{("arrival","start","arrival.court"),("temple","temple.entry","temple.exit"),("ascent","boost-landing","final.06")}),
                ("module.004.solar-foundry","m04.",new[]{("intake","intake.start","transfer.dock"),("outer","transfer.exit","furnace.court"),("crown","cooling.safe","patch.base")})})
            {
            ModuleSelectionState.Select(world.Item1,true);
            ...
            var all=module.Blocks.Where(b=>!b.StableId.Contains("mastery")&&!b.StableId.Contains("risky"))
                .Concat(module.CrumblingBlocks.Select(b=>new ModuleBlockDefinition(b.StableId,b.Pose.Position,b.Size,ModuleMaterialRole.Crumbling)))
                .OrderBy(b=>b.Pose.Position.z).ToArray();
            ...
            foreach(var bounds in world.Item3)
            foreach(string mode in new[]{"slow","flow","expert"})
            foreach(string error in new[]{"clean","left","right","early","late","heading"})
            {
                int first=System.Array.FindIndex(all,b=>b.StableId==world.Item2+bounds.Item2),last=System.Array.FindIndex(all,b=>b.StableId==world.Item2+bounds.Item3);
                var sequence=all.Skip(first).Take(last-first+1).ToArray();
                float speed=mode=="slow"?7.02f:mode=="expert"?17.5f:13.5f;
                ...
                var entryDirection=Vector3.ProjectOnPlane(sequence[1].Pose.Position-sequence[0].Pose.Position,Vector3.up).normalized;
                motor.ApplyLaunch(Quaternion.Euler(0,error=="heading"?5:0,0)*entryDirection*speed,true);input.JumpHeld=mode!="slow";
                ...
                if(!reached)failures.Add(bounds.Item1+"/"+mode+"/"+error+" at "+motor.transform.position+" lands="+landings+" fatal="+motor.GetComponent<SceneryLandingRecovery>().LastFatalCollider?.transform.parent?.name);
                yield return null;
            }
            Directory.CreateDirectory("Logs/WorldReauthorQA");File.WriteAllLines("Logs/WorldReauthorQA/"+world.Item1+"-held.csv",rows);File.WriteAllLines("Logs/WorldReauthorQA/"+world.Item1+"-outcomes.csv",outcomes);
            allFailures.AddRange(failures.Select(f=>world.Item1+"/"+f));
            yield return new UnitySceneLevelLoader().LoadAsync("ModuleSelector");
            }
            Assert.That(allFailures,Is.Empty,string.Join("\n",allFailures));
        }
    }
}
